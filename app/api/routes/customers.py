...
def get_customer(customer_id: int, session: Session = Depends(get_db)) -> CustomerResponse:
    try:
        return get_customer_tool(session, GetCustomerInput(customer_id=customer_id))
...
def get_customer_orders(
    customer_id: int, session: Session = Depends(get_db)
) -> list[OrderResponse]:
    try:
        return get_customer_orders_tool(session, GetCustomerInput(customer_id=customer_id))
    except ToolError as error:
        raise_http_for_tool_error(error)
...
def get_customer_tickets(
    ...
    try:
        return get_customer_tickets_tool(session, GetCustomerInput(customer_id=customer_id))
    except ToolError as error:
        raise_http_for_tool_error(error)
