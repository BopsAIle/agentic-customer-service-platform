...
def make_revalidate_node(session: Session) -> Callable[[AgentState], AgentState]:
    def revalidate(state: AgentState) -> AgentState:
        ...
        if action is None or action.status != PendingActionStatus.CONFIRMED:
            return _failed(
                state, AgentErrorCategory.POLICY_DENIED, "There is no confirmable pending action."
            )
        if action.conversation_id != state.get(
            "conversation_id"
        ) or action.customer_id != state.get("customer_id"):
            return _failed(
