...
def build_graph(
    ...
    *,
    ...
    graph.add_node("check_pending_action", cast(Any, make_check_pending_node(clock, ttl_seconds)))
    graph.add_node("understand_request", cast(Any, make_understand_request_node(provider)))
    graph.add_node("select_tool_or_response", select_tool)
    graph.add_node("validate_tool", validate_tool)
    graph.add_node(
        "evaluate_policy", cast(Any, make_evaluate_policy_node(policy_engine, audit_log, clock))
    )
    graph.add_node("inspect_risk", _inspect_risk)
    graph.add_node("create_pending_action", cast(Any, make_create_pending_node(clock, audit_log)))
    graph.add_node("policy_revalidation", cast(Any, make_revalidate_node(session)))
    graph.add_node("execute_tool", cast(Any, make_confirmed_execution_node(session)))
    graph.add_node("execute_confirmed_action", cast(Any, make_confirmed_execution_node(session)))
    graph.add_node("execute_human_escalation", cast(Any, make_human_escalation_node(session)))
    graph.add_node(
        "retrieve_knowledge",
        cast(Any, make_retrieve_node(knowledge_retriever, grounded_generator)),
    )
