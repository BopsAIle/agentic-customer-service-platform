...
function runStages(scenario: DemoScenario): ShowcaseStage[] {
  ...
  return [
    ...
    { id: "authority", label: "Authority boundary", status: clarificationRequired || confirmationWaiting ? "waiting" : blocked ? "blocked" : "completed", evidence: clarificationRequired ? "Clarification required" : confirmationWaiting ? "Confirmation required" : blocked ? "Authority denied" : "Authority state recorded", authority: "No authority granted", explanation: clarificationRequired ? "The request remains non-executable until the required information is supplied." : confirmationWaiting ? "The proposal is eligible but remains behind explicit confirmation." : blocked ? "Deterministic controls prevented authority escalation or duplicate effect." : "Only controlled runtime paths can hold execution authority." },
    { id: "execution", label: "Execution", status: executionCompleted ? "completed" : clarificationRequired ? "waiting" : "blocked", evidence: executionCompleted ? "Effect recorded" : clarificationRequired ? "Execution not attempted" : "Execution prevented", authority: executionCompleted ? "Controlled execution" : "No authority granted", explanation: executionCompleted ? "A controlled runtime path recorded the effect." : clarificationRequired ? "No mutation was attempted because the request was incomplete." : "The authority boundary prevented a business mutation in this evidence snapshot." },
  ];
}
...
function scenarioDescription(scenario: DemoScenario) {
  ...
  if (scenario.scenario_id === "prompt-injection-defense") return "Untrusted input reaches policy evaluation, is denied, and receives no authority.";
  if (scenario.scenario_id === "duplicate-operation-protection") return "A repeated request meets idempotency protection before it can create a second effect.";
...
function InvestigationSummary({ scenario, onReport }: { scenario: DemoScenario; onReport: () => void }) {
  ...
  const outcome = clarificationRequired ? "Clarification required" : waiting ? "Waiting confirmation" : denied ? "Blocked" : humanize(run.status);
  ...
  return <section className="showcase-investigation-summary" aria-label="Investigation summary"><div className="showcase-investigation-summary-heading"><div><div className="eyebrow">Investigation summary</div><h2>{scenario.title}</h2><p>{scenarioDescription(scenario)}</p></div><button type="button" className="showcase-report-button" onClick={onReport}>View investigation report</button></div><div className="showcase-summary-grid"><div><span>Outcome</span><strong>{outcome === "Waiting confirmation" ? "Awaiting confirmation" : outcome === "Blocked" ? "Prevented" : outcome}</strong></div><div><span>Evidence collected</span><strong>{evidenceCount}</strong></div><div><span>Decision</span><strong>{decision}</strong></div><div><span>Execution</span><strong>{clarificationRequired ? "Not attempted" : waiting ? "Awaiting confirmation" : denied ? "Prevented" : "Not authorized"}</strong></div><div><span>Authority</span><strong>{clarificationRequired ? "Not authorized" : waiting ? "Awaiting confirmation" : "Not authorized"}</strong></div></div></section>;
}
