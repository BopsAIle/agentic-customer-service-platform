...
function exportInvestigationReport(run: AgentRun) {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>Investigation report · ${escapeHtml(run.run_id)}</title><style>body{font:14px system-ui,sans-serif;color:#17202b;background:#f6f8fa;max-width:980px;margin:0 auto;padding:32px}section{background:#fff;border:1px solid #dbe2e8;border-radius:10px;padding:20px;margin:16px 0}h1,h2{margin:0 0 8px}h1{font-size:24px}h2{font-size:17px}p,li{line-height:1.55;color:#536170}.meta{font-family:ui-monospace,monospace;color:#536170}.stage{border-top:1px solid #edf0f2;padding:12px 0}.stage:first-child{border-top:0}.status{font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:#286b4d}.unavailable{color:#7a8793;font-style:italic}.label{font-size:11px;text-transform:uppercase;letter-spacing:.1em;color:#7a8793}</style></head><body><div class="label">Agent control plane · bounded investigation export</div><h1>Run investigation report</h1><p class="meta">${escapeHtml(run.run_id)} · ${escapeHtml(new Date(run.started_at).toLocaleString())}</p><section><h2>Run overview</h2><p><strong>Status:</strong> ${reportValue(run.status)}<br><strong>Intent:</strong> ${reportValue(run.intent)}<br><strong>Request:</strong> <span class="unavailable">Not provided by backend projection</span></p></section><section><h2>Decision timeline</h2>${stages.map((stage) => `<div class="stage"><strong>${escapeHtml(stage.label)}</strong> <span class="status">${escapeHtml(stage.status)}</span><p>${escapeHtml(stage.explanation)}</p>${stage.evidence ? `<p class="meta">Evidence: ${escapeHtml(stage.evidence)}</p>` : ""}</div>`).join("")}</section><section><h2>Grounding evidence</h2>${run.rag_documents.length ? `<ul>${run.rag_documents.map((document) => `<li>${escapeHtml(document.title)} · ${escapeHtml(document.source)} · ${escapeHtml(document.citation_id)}${Number.isFinite(document.score) ? ` · score ${document.score}` : ""}</li>`).join("")}</ul>` : `<p class="unavailable">Evidence unavailable in current projection.</p>`}<p><strong>Verified facts:</strong> <span class="unavailable">Not provided by backend projection.</span></p></section><section><h2>Policy and execution</h2><p><strong>Policy decision:</strong> ${reportValue(policy?.outcome)}<br><strong>Policy reason:</strong> ${policy?.reason_codes.length ? escapeHtml(policy.reason_codes.join(" · ")) : "Not provided by backend projection"}<br><strong>Confirmation:</strong> ${run.status === "waiting_confirmation" ? "Waiting for confirmation" : "Not provided by backend projection"}<br><strong>Execution:</strong> ${run.status === "waiting_confirmation" ? "Not executed" : reportValue(run.tools[0]?.status)}<br><strong>Safety status:</strong> ${run.status === "error" ? "Blocked / failed" : "No violation recorded in this projection"}</p></section><footer><p class="unavailable">This report contains bounded operator projection fields only. Prompts, raw provider responses, secrets, and hidden reasoning are excluded.</p></footer></body></html>`;
...
function DecisionContextSummary({ run }: { run: AgentRun }) {
  ...
  const execution = run.status === "waiting_confirmation" || writeStatus === "pending_confirmation" || writeStatus === "not_executed" ? "No mutation" : humanize(writeStatus);
