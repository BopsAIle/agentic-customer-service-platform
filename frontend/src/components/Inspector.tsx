...
function ActionInspector({ run }: { run: AgentRun }) {
  ...
  const isPending = run.evidence.confirmation.status === "pending" || run.evidence.write_outcome.status === "pending_confirmation" || run.status === "waiting_confirmation" || policy?.confirmation_status === "pending";
  const isExecuted = run.evidence.write_outcome.status === "executed" || tool?.status === "executed";
