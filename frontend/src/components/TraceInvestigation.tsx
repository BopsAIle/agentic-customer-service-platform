...
export function DecisionExplanationCard({ run }: { run: AgentRun }) {
  ...
  const blocking = policy?.outcome === "deny"
    ...
    ? [run.decision_reason ?? "Deterministic controls prevented execution"]
    : [];
