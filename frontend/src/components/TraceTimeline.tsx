...
export function TraceStageTimeline({ run, compact = false }: { run: AgentRun; compact?: boolean }) {
  ...
  return <div className={`trace-stage-layers ${compact ? "trace-stage-list-compact" : ""}`}>{layers.map(({ layer, stages: layerStages }) => <section className="trace-layer" key={layer}><div className="trace-layer-heading"><span className="eyebrow">{layerLabels[layer]}</span><span className="trace-layer-rule" aria-hidden="true" /></div><div className="trace-stage-list">{layerStages.map((stage, index) => <details className={`trace-stage trace-stage-${stage.status}`} key={stage.id} open={index === 0 || stage.status === "waiting" || stage.status === "failed"}><summary className="trace-stage-summary"><span className="trace-stage-marker"><StageIcon status={stage.status} /></span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><span className="truncate text-sm font-medium text-main">{stage.label}</span>{stage.contextRole && <Badge tone={contextRoleLabels[stage.contextRole].tone}>{contextRoleLabels[stage.contextRole].label}</Badge>}</span><span className="mt-1 block text-xs text-muted">{stage.explanation}</span></span><span className="trace-stage-meta"><StatusIndicator label={stage.status.replace(/_/g, " ")} tone={stageTone(stage.status)} compact /><span className="hidden font-mono text-[10px] text-muted sm:inline">{stageMeta(stage)}</span></span></summary><div className="trace-stage-detail"><div className="trace-stage-explanation"><span className="field-label">System explanation</span><p className="mt-1 text-xs leading-5 text-muted">{stage.explanation}</p></div><div className="trace-stage-operational-meta"><div><span className="field-label">Duration</span><strong>{formatTraceDuration(stage.metadata)}</strong></div><div><span className="field-label">Evidence</span><strong>{stage.metadata.source === "not_recorded" ? "Not recorded" : stage.metadata.evidenceCount}</strong></div><div><span className="field-label">Owner</span><strong>{stage.metadata.owner}</strong></div><div><span className="field-label">Metadata source</span><strong>{stage.metadata.source === "recorded_fixture" ? "Recorded fixture" : stage.metadata.source === "observed" ? "Observed event" : "Not recorded"}</strong></div></div>{stage.evidence && <div className="trace-stage-evidence"><span className="field-label">Observed evidence</span><p className="mt-1 font-mono text-[11px] text-info">{stage.evidence}</p></div>}{stage.events.length > 0 && <div className="mt-3 space-y-1.5">{stage.events.map((event, eventIndex) => <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border bg-void/30 px-2.5 py-2 text-[11px]" key={`${event.name}-${event.timestamp}-${eventIndex}`}><div><span className="block text-xs font-medium text-main">{event.event_key ? eventKeyLabel(event) : eventLabel(event.name)}</span><span className="mt-1 block font-mono text-[10px] text-muted">{event.name}</span>{event.metadata && Object.keys(event.metadata).length > 0 && <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-info">{Object.entries(event.metadata).map(([key, value]) => <span key={key}>{key.replace(/_/g, " ")}: {String(value).replace(/_/g, " ")}</span>)}</div>}</div><span className="inline-flex items-center gap-1 text-muted"><Clock3 size={11} aria-hidden="true" />{new Date(event.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })} · {event.duration_ms.toFixed(1)} ms</span></div>)}</div>}</div></details>)}</div></section>)}</div>;
}
