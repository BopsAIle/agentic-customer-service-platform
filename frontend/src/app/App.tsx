import { Activity, Bot, Clock3, Command, ShieldCheck } from "lucide-react";
...
export function App() {
  ...
  const send = async (message: string) => { setBusy(true); setError(null); try { const response = await api.chat(conversationId, customerId, message); setTurns((current) => [...current, { request: message, response }]); const nextRun = await api.run(response.agent_run_id); setRun(nextRun); setMemory(await api.memory(customerId)); const nextHealth = await api.health(); setHealth(nextHealth); setApiReachability("reachable"); } catch (caught) { if (caught instanceof ApiError && caught.status === 401) { setAuth(getApiAuthSnapshot()); setTurns([]); setRun(null); setMemory([]); setHealth(null); setApiReachability("unknown"); } else if (caught instanceof TypeError) { setHealth(null); setApiReachability("unavailable"); } setError(caught instanceof Error ? caught.message : "The operator API is unavailable."); } finally { setBusy(false); } };
