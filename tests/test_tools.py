...
def test_cancel_pending_and_processing_orders(db_session: Session) -> None:
    pending = cancel_order(db_session, CancelOrderInput(customer_id=1, order_id=3))
    processing = cancel_order(db_session, CancelOrderInput(customer_id=1, order_id=4))
...
def test_cancel_rejects_shipped_and_delivered(db_session: Session, order_id: int) -> None:
    with pytest.raises(InvalidStateTransitionError):
        cancel_order(db_session, CancelOrderInput(customer_id=1, order_id=order_id))


def test_cancel_is_idempotent(db_session: Session) -> None:
    first = cancel_order(db_session, CancelOrderInput(customer_id=1, order_id=3))
    second = cancel_order(db_session, CancelOrderInput(customer_id=1, order_id=3))
...
def test_cancel_rejects_cross_customer_order(db_session: Session) -> None:
    with pytest.raises(OwnershipError):
        cancel_order(db_session, CancelOrderInput(customer_id=2, order_id=3))
