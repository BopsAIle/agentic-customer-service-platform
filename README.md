...
or changes a collection. `RAG_BACKEND=local` does not require Qdrant. These structural checks
cannot prove that an equal-dimension collection was generated by the exact configured embedding
model because model identity is not persisted in the collection.
